...
export default {
  ...
  methods: {
    ...
    deleteProject: function (entry, removedAlias) {
      this.$q.dialog({
        ...
        .onOk(() => {
          if (this.projectList.length < 1) {
            ...
            if (this.uuidList.includes(entry)) {
              this.db.collection('projects').doc(entry).delete()
                .then(() => {
                  this.$q.notify({
                    icon: 'done',
                    color: 'positive',
              ...
              this.db.collection('projects').doc('ToC').update(updates)
              ...
              this.cloudFunctions({ folder: `projects/${entry}` })
            } else {
              this.$q.dialog({
                title: 'Error',
